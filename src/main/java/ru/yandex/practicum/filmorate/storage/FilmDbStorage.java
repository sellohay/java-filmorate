...
public class FilmDbStorage implements FilmStorage {
    ...
    private static final String SELECT_QUERY = "SELECT f.film_id, f.name, f.description, f.release_date, f.duration, " +
                    "m.mpa_rating_id, m.rating_name FROM films f LEFT JOIN mpa_rating m ON f.mpa = m.mpa_rating_id";
    private static final String GET_FILM_BY_ID = "SELECT f.film_id, f.name, f.description, f.release_date, " +
                    "f.duration, m.mpa_rating_id, m.rating_name FROM films f " +
                    "LEFT JOIN mpa_rating m ON f.mpa = m.mpa_rating_id WHERE f.film_id = ?";
    ...
    private static final String SELECT_POPULAR_FILMS = "SELECT f.film_id, f.name, f.description, f.release_date, " +
            "f.duration, m.mpa_rating_id, m.rating_name FROM films f " +
            "LEFT JOIN mpa_rating m ON f.mpa = m.mpa_rating_id LEFT JOIN film_likes fl ON f.film_id = fl.film_id " +
            "GROUP BY f.film_id, f.name, f.description, f.release_date, f.duration, " +
            "m.mpa_rating_id, m.rating_name ORDER BY COUNT(fl.film_likes_id) DESC LIMIT ?";
    ...
    public Collection<Film> getFilms() {
        return jdbcTemplate.query(SELECT_QUERY, filmRowMapper);
    }
    ...
    public List<Film> getPopularFilms(int count) {
        try {
            return jdbcTemplate.query(SELECT_POPULAR_FILMS, filmRowMapper, count);
        } catch (EmptyResultDataAccessException ignored) {
            return List.of();
        }
    }
